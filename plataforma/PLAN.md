...
- [x] **(2026-07-24) Timer de limpieza de jugadores inactivos del Quiz**: mismo patrón autocurativo que el guard de arriba, esta vez disparando la Lambda `quiz-cleanup` una vez por día (borra el historial de jugadores sin actividad en 30+ días que no estén en ningún top 20 — ver `proyectos/quiz/docs/ARQUITECTURA.md` §16). `plataforma/scripts/quiz-cleanup-invoke.sh` invoca la Lambda por HTTP directo contra el puerto local de Floci (sin `aws-cli` en el VPS) + `plataforma/systemd/quiz-cleanup.{service,timer}` (`OnCalendar=daily`), instalado igual que el guard. `quiz-avanzado` usa un disparador distinto a propósito (EventBridge Scheduler real, ver `proyectos/quiz-avanzado/docs/GUIA-SERVICIOS-AVANZADOS.md` §5) — es el único proyecto donde corresponde documentar servicios avanzados de AWS.
